...
| R4 Overseer MVP | Next | Local transcript normalizer, comparison summaries, decision packets, task assignment, human approval queue (no auto-pass) | Golden fixtures for supported clients; redaction and provenance tests |
| R4.1 Standing auto-pass + notify | After R4 | Human-authored standing policies; mechanical auto-pass only on hard match; notify + queue on questionable/escalate ([ADR-003](decisions/ADR-003-overseer-standing-auto-pass-and-notifications.md)) | Default-deny evaluator tests; policy-hash binding; reason codes; no silence-as-approval |

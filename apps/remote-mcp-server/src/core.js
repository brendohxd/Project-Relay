...
function validateAppendInput(input) {
  ...
  if (input.expectedPreviousHash !== null && input.expectedPreviousHash !== undefined && !HASH_PATTERN.test(input.expectedPreviousHash)) {
    throw Object.assign(new Error("expectedPreviousHash must be null or a SHA-256 digest"), { code: "INVALID_INPUT" });
  }
  ...
  return {
    async append(input, principal) {
      ...
      if (prior) {
        ...
        return { outcome: "replayed", record: publicRecord(prior), network_actions_performed: [] };
      }
      ...
      return { outcome: "applied", record: publicRecord(row), network_actions_performed: [] };
    },
